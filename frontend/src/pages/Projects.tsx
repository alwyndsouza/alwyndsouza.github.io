...
export function Projects() {
  ...
  const inDevelopment = filtered.filter(p => p.status !== 'production');
    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      {production.length > 0 && (
        <section className="mb-12">
          <h2 className="text-2xl font-bold mb-1">Production</h2>
          <p className="text-sm text-muted-foreground mb-5">Live and shipped projects.</p>
          <div className="space-y-4">
            {production.map((project, i) => <ProjectCard key={project.id} project={project} i={i} />)}
          </div>
        </section>
      )}
      ...
      {inDevelopment.length > 0 && (
        <section>
          <h2 className="text-2xl font-bold mb-1">In Development</h2>
          <p className="text-sm text-muted-foreground mb-5">Works in progress.</p>
          <div className="space-y-4">
            {inDevelopment.map((project, i) => <ProjectCard key={project.id} project={project} i={i} />)}
          </div>
        </section>
      )}
    </div>
